...
// Na v3, usamos queryCollection. Como não configuramos schema complexo, 
// pegamos tudo e podemos limitar ou ordenar se necessário.
  ...
  <div class="bg-charcoal min-h-screen pt-32 pb-24 selection:bg-gold/30 selection:text-charcoal-dark">
    <div class="container mx-auto px-6 max-w-7xl">
      ...
      <header class="mb-16 md:mb-24">
        <NuxtLink to="/#blog" class="inline-flex items-center gap-2 text-[10px] font-bold tracking-[0.3em] uppercase text-white/40 hover:text-gold transition-colors mb-8">
      ...
      <div v-if="posts && posts.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        <article v-for="post in posts" :key="post.path" class="group flex flex-col bg-[#14120E] border border-white/5 hover:border-gold/30 hover:shadow-2xl hover:shadow-black/50 transition-all duration-500 overflow-hidden">
          
          <NuxtLink :to="post.path" class="relative aspect-[16/10] overflow-hidden block">
            <img :src="post.image || 'https://images.unsplash.com/photo-1589829085413-56de8ae18c73?auto=format&fit=crop&q=80&w=800'" :alt="post.title" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105 opacity-80 group-hover:opacity-100" />
            <div class="absolute inset-0 bg-charcoal/20 group-hover:bg-transparent transition-colors duration-500"></div>
            <div v-if="post.category" class="absolute top-4 left-4 bg-charcoal/90 backdrop-blur-sm border border-white/10 px-4 py-1.5 text-[9px] font-bold tracking-[0.2em] uppercase text-gold">
              {{ post.category }}
            </div>
          </NuxtLink>
          ...
          <div class="p-8 flex flex-col flex-grow">
            ...
            <NuxtLink :to="post.path" class="mt-auto inline-flex items-center gap-2 text-[10px] font-bold tracking-[0.3em] uppercase text-gold hover:text-white transition-colors w-fit border-b border-gold/30 hover:border-white pb-1">
              Ler Artigo
